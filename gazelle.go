...
func (e *Extension) RegisterFlags(fs *flag.FlagSet, cmd string, c *config.Config) {
	e.ccTestDependencyFlag = fs.String("cc_test_dependency", "", "dependency to run to all cc_tests")
}
...
func (e *Extension) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, importsRaw interface{}, from label.Label) {
	...
	r.DelAttr("deps")
	var deps []string
	if r.Kind() == "cc_test" && *e.ccTestDependencyFlag != "" {
		deps = make([]string, 0, len(imports)+1)
		deps = append(deps, *e.ccTestDependencyFlag)
	} else {
